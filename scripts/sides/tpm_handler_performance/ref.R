# A shortened version of the ref-script to only test performance
...
counts <- read.csv("cropped.countTable", header=TRUE, sep="\t", row.names = 1)
gene_lengths <- read.csv("v22.lengths", header=TRUE, sep="\t", row.names = 1)
...
library('tibble')
library('dplyr')
write.table(rownames_to_column(as.data.frame(TPMs),'geneID') %>% mutate_if(is.numeric, round, digits = 5),file="cropped.countTable.TPM", sep="\t", quote = FALSE, row.names=FALSE)

# this part below is for making a second file with gene names instead of ids (since tpm_handler does not do this by default we skip this)
# # Replace ensembl codes with gene symbols:
# gene_names <- read.csv("v22.info", header=TRUE, sep="\t", colClasses=c("NULL", "NULL", "NULL", "NULL", NA, "NULL", NA), row.names = 5)
...
# # Save to file
# write.table(TPMs_gnames %>% mutate_if(is.numeric, round, digits = 5),file="cropped.countTable.TPM.gnames_sum_dup", sep="\t", quote = FALSE, row.names=FALSE)